...
export default function StartMeeting() {
  ...
  return (
    <div className='flex-1 px-2 md:px-8 py-10 flex flex-col gap-10'>
      <div className='bg-[#BBE9FF] p-10 rounded-xl text-black'>
        <h1 className='text-[30px] font-extrabold capitalize py-4'>Create your custom room ID</h1>
        <input type="text" className='w-full outline-none bg-white border-[1px] border-[#669bbc] rounded-md py-4 px-6 text-[20px] font-medium placeholder:capitalize' placeholder='Enter your custom rooom id' onChange={(e) => {
        ...
        <div className='w-full' style={{ display: isValid ? "block" : "none" }}>
          <div className='flex w-full justify-between py-10'>
            <section className='w-[48%] p-4 border-2 border-dashed border-[#e0e1dd] rounded-md'>
            ...
            <section className='w-[48%] p-4 border-2 border-dashed border-[#e0e1dd] rounded-md'>
              ...
              <div className='flex items-center gap-4'>
                <p className='py-3 text-[20px] font-light'>{`videocallingapp-one.vercel.app/meeting/${roomID}`}</p>
                <Image src={"/images/copy.svg"} alt='copy image here' width={25} height={25} className=' opacity-65 cursor-pointer' onClick={async () => {
                  await navigator.clipboard.writeText(`https://videocallingapp-one.vercel.app/meeting/${roomID}`)
                  alert("Room ID Copied To Clipboard!")
                }} />
              </div>
            </section>
